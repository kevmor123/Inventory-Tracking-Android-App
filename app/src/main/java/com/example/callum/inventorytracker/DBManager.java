...
public class DBManager extends SQLiteOpenHelper{
    ...
    @Override//called when db structur eis changed
    ...
    //Deletes project and associated items from db
    ...
    public void addEquipment(EquipmentItem equipment){//adds new equipments to equipment table
        ...
        c.put(COLUMN_DAMAGED,equipment.isDamaged()? 1:0);
        SQLiteDatabase db = getWritableDatabase();
    ...
    //Function to check if a barcode is already in the DB
    ...
    //Searches db for a query. Returns any tuple starting with the search query
    public EquipmentItem[] searchItems(String search){
        EquipmentItem[] items;
        String query = "SELECT * FROM " + TABLE_EQUIPMENT +
                " WHERE " + COLUMN_EQUIPMENT_NAME + " LIKE '"+search+"%"+"'"+
                " ORDER BY "+COLUMN_EQUIPMENT_NAME+" ASC";
        SQLiteDatabase db = getReadableDatabase();
        EquipmentItem equipment;
        Cursor c = db.rawQuery(query,null);
